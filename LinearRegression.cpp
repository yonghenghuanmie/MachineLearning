...
void GradientDescent(Eigen::MatrixXd& model, const Eigen::MatrixXd& train_input, const Eigen::MatrixXd& train_output, const Eigen::VectorXd& learning_rate, size_t batch_size)
{
	for (size_t i = 0; i < batch_size; i++)
	{
		for (size_t train_index = 0; train_index < train_input.rows(); train_index++)
		{
			...
			std::cout << Eigen::MatrixXd(learning_rate.array() * (1.0 / train_input.rows() * hx_sub_jx.row(train_index) * duplicate_line).transpose().array()) << "\n\n";*/
			model += Eigen::MatrixXd(learning_rate.array() * (1.0 / train_input.rows() * hx_sub_jx.row(train_index) * duplicate_line).transpose().array());
			std::cout << model << "\n\n";
		}
	}
}
...
int main()
{
	...
	Eigen::VectorXd learning_rate = Eigen::Vector2d(0.001, 0.25);
	int batch_size = 100;
